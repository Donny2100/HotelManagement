...
namespace MC.ORM
{
    public class Database : IDisposable, IDatabase
    {
        ...
        /// <summary>
        /// 构造使用配置文件：app.config/web.config中的第一个连接字串
        /// </summary>
        /// <exception cref="InvalidOperationException">当没有数据库连接字串可以注册时抛出异常</exception>
        public Database()
        {
            if (ConfigurationManager.ConnectionStrings.Count == 0)
                throw new InvalidOperationException("一个或多个连接字符串必须被注册");
        ...
        /// <summary>
        ///     构造器使用所提供的连接字串名称和provider名称
        ///     如果没有指出provider名称，默认为MS SQL Server
        /// </summary>
        ...
        public Database(string connectionString, string providerName = null)
        {
            if (string.IsNullOrEmpty(connectionString))
                throw new ArgumentException("数据库连接字串不能为空!", "connectionString");
        ...
        public Database(string connectionStringName)
        {
            if (string.IsNullOrEmpty(connectionStringName))
                throw new ArgumentException("数据库连接字串不能为空!", "connectionStringName");

            var entry = ConfigurationManager.ConnectionStrings[connectionStringName];

            if (entry == null)
                throw new InvalidOperationException(string.Format("找不到连接字串： '{0}'", connectionStringName));
        ...
        /// <param name="sqlCount">记录总数的sql语句</param>
        /// <param name="countArgs">记录总数的sql语句参数</param>
        /// <param name="sqlPage">列表的sql语句</param>
        /// <param name="pageArgs">列表的sql语句的参数</param>
        /// <returns>某一页的数据列表和记录总数等</returns>
        /// <remarks>
        ///    返回翻页列表对像
        ///    其中包括翻页要使用的所有数据
        /// </remarks>
        ...
        /// <param name="page">从1开始的页码</param>
        /// <param name="itemsPerPage">每页数量</param>
        /// <param name="sqlCount">取回数据总记录数的sql语句</param>
        /// <param name="sqlPage">列表的sql语句</param>
        /// <returns>某一页的数据列表和记录总数等</returns>
        /// <remarks>
        ///    返回翻页列表对像
        ...
        #region 执行: Fetch (page)操作

        /// <summary>
        ///     返回某一页的数据列表，不带记录总数
        /// </summary>
        /// <typeparam name="T">类型</typeparam>
        /// <param name="page">从1开始的某一页</param>
        ...
        /// </summary>
        /// <param name="tableName">表名</param>
        /// <param name="primaryKeyName">表的主健</param>
        /// <param name="autoIncrement">主健是否自动编号</param>
        /// <param name="poco">models</param>
        /// <returns>The auto allocated primary key of the new record, or null for non-auto-increment tables</returns>
        /// <remarks>
        ...
        /// </summary>
        /// <param name="poco">models</param>
        /// <returns>自动编号的表返回新id，非自动编号的表返回null</returns>
        /// <remarks>
		...
		public object Add(object poco, bool autoIncrement = true)
		{
            if (poco == null)
                throw new ArgumentNullException("poco");

            var pd = PocoData.ForType(poco.GetType(), _defaultMapper);
            return ExecuteAdd(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, autoIncrement, poco);
        }
        private object ExecuteAdd(string tableName, string primaryKeyName,bool autoIncrement, object poco)
		{
			try
			{
				OpenSharedConnection();
				try
				{
					using (var cmd = CreateCommand(_sharedConnection, ""))
					{
						...
						if (autoIncrement)
						{
							begin = SingleOrDefault<MaxId>($"select max({primaryKeyName}) as id from {_provider.EscapeTableName(tableName)};");
						}
						...
						if (autoIncrement)
						{
							var end = SingleOrDefault<MaxId>($"select max({primaryKeyName}) as id from {_provider.EscapeTableName(tableName)};");
                            //if (end.Id > begin.Id)
                            //	return end.Id;
                            //return null;
                            return end.Id;
        ...
        /// </summary>
        /// <param name="tableName">表名</param>
        /// <param name="primaryKeyName">表的主健名</param>
        /// <param name="poco">models</param>
        /// <param name="primaryKeyValue">主健值</param>
        /// <returns>受影响的行数</returns>
        ...
        /// </summary>
        /// <param name="tableName">表名</param>
        /// <param name="primaryKeyName">主健名</param>
        /// <param name="poco">models</param>
        /// <param name="primaryKeyValue">主健值</param>
        /// <param name="columns">需要更新的列，为空时更新所有列</param>
        /// <returns>受影响的行数</returns>
        ...
        {
            if (string.IsNullOrEmpty(sql))
                throw new ArgumentNullException("sql为空");
        ...
        ///    执行删除delete操作
        /// </summary>
        /// <param name="poco">models，需要指定指定主健值</param>
        /// <returns>受影响的行数</returns>
        ...
        /// <param name="pocoOrPrimaryKey">主健值</param>
        /// <returns>受影响的行数</returns>
        ...
        /// </summary>
        /// <typeparam name="T">实体类型</typeparam>
        /// <param name="sql">删除sql语句，可以不指定Delete from table</param>
        ...
        /// <typeparam name="T">实体类型</typeparam>
        /// <param name="sql">
        ///     删除sql语句，可以不指定Delete from table
        /// </param>
        /// <returns>受影响的行数</returns>
